...
export class Overview extends React.Component<OverviewProps, OverviewState> {
  ...
  private renderYourVehicles(): JSX.Element
  {
    ...
    const onClick = this.loaded() ? (carID: string): void => this.handleRowClick(carID, 'userVehicles') : undefined;
  ...
  private renderNetworkVehicles(): JSX.Element
  {
    ...
    const onClick = this.loaded() ? (carID: string): void => this.handleRowClick(carID, 'networkVehicles') : undefined;
  ...
  private renderCreateVehiclePopup(): JSX.Element
  {
    const content = (
      <form id="createVehicleForm" onSubmit={this.createVehicle.bind(this)} className="createVehicleForm" >
        <label>ID:</label>
        <input type="text" onChange={(e): void => this.handleFormEntry(e, 'id')} value={this.state.createVehicleForm.id} />
        <label>Make:</label>
        <input type="text" onChange={(e): void => this.handleFormEntry(e, 'make')} value={this.state.createVehicleForm.make} />
        <label>Model:</label>
        <input type="text" onChange={(e): void => this.handleFormEntry(e, 'model')} value={this.state.createVehicleForm.model} />
        <label>Colour:</label>
        <input type="text" onChange={(e): void => this.handleFormEntry(e, 'colour')} value={this.state.createVehicleForm.colour} />
        <label>Owner:</label>
        <input type="text" onChange={(e): void => this.handleFormEntry(e, 'owner')} value={this.state.createVehicleForm.owner} />
      </form>
    );
    ...
    return (
      <Popup title="create car" content={content} popupType={PopupType.CancelSubmit}
        ...
        onSecondary={(): void => this.setState({ showCreateVehicle: false })}
      />
    );
  }

  private showCreateVehiclePopup(): void
  {
    let carId = 'CAR9999';

    if (this.props.networkVehicles) {
      const offset = 11; // skip protected values
  ...
  private handleFormEntry(e: React.ChangeEvent<HTMLInputElement>, type: keyof OverviewState['createVehicleForm']): void
  {
    const obj: { [K in keyof OverviewState['createVehicleForm']]?: string } = {};
  ...
  private async createVehicle(e: React.FormEvent<HTMLFormElement>): Promise<void>
  {
    e.preventDefault();
    ...
    try {
      ...
      const err: Error = error as Error; // limit the scope of the any
      console.log(err);
  ...
  private handleRowClick(carID: string, table: 'userVehicles' | 'networkVehicles'): void
  {
    if (this.loaded()) {
