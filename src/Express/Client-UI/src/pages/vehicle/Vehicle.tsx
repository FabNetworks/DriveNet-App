...
export class Vehicle extends React.Component<VehicleProps, VehicleState> {
    ...
    private renderNewOwnerPopup(): JSX.Element {
        ...
        return (
            <Popup title="change owner" content={content} popupType={PopupType.CancelSubmit}
                ...
                onSecondary={() => this.setState({showNewOwner: false})}
            />
        )
    }
    ...
    private renderDeleteVehiclePopup(): JSX.Element {
        return (
            <Popup title="delete car?" content={`Are you sure you wish to delete ${this.props.vehicle.key}?`}
                ...
                onSecondary={() => this.setState({showDeleteVehicle: false})}
            />
        )
    }

    private showNewOwnerPopup() {
        this.setState({
            showNewOwner: true
        });
    }

    private showDeleteVehiclePopup() {
        this.setState({
    ...
    private handleOwnerEntry(e: React.ChangeEvent<HTMLInputElement>) {
        this.setState({
            newOwner: e.currentTarget.value
        });
    }

    private async changeOwner(e: React.FormEvent<HTMLFormElement>) {
        e.preventDefault();
