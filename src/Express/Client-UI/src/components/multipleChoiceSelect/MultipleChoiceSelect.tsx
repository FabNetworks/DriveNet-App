        ...
        return (
            <div className={`multipleChoiceSelect-component ${this.state.showingMenu ? 'show' : ''}`} >
                ...
                <div className="choices" >
                    <input id={`${this.state.id}-searchText`} type="text" placeholder="search" onChange={(e) => this.setState({searchText: e.currentTarget.value})} value={this.state.searchText} />
                    <ul ref={this.ulRef} className={`${this.state.ulOverflowing ? 'overflowing' : ''}`} >
                        { this.props.choices.filter((choice) => choice.toString().toLowerCase().startsWith(this.state.searchText.toLowerCase())).map((choice) => {
                            return (
                                <li id={`${this.state.id}-${choice}`} key={choice} tabIndex={0} onKeyPress={(e) => this.handleKeyPress(e, () => this.handleSelect(!this.state.selected.includes(choice), choice))} >
                                    <input type="checkbox" id={`${this.state.id}-${choice}-checkbox`} onChange={(e) => this.handleSelect(e.currentTarget.checked, choice)} checked={this.state.selected.includes(choice)} />
                                    <label id={`${this.state.id}-${choice}-label`} htmlFor={`${this.state.id}-${choice}-checkbox`} >{choice}</label>
                                </li>
                            )
                        })}
                    </ul>
