...
export class Table<T extends string> extends React.Component<TableProps<T>, TableState<T>> {
            ...
            <div className="options" >
                ...
                {numPerPageOptions[0] < this.props.data.length &&
                    <span className="perPage" >
                        Showing&nbsp;
                        <Select onSelect={(choice) => this.setState({numPerPage: choice as number, currentPage: 0})} choices={numPerPageOptions} />
                        &nbsp;per page
                    </span>
                }
            </div>
    ...
    private renderSelectFilter(column: ColumnProps<T>): JSX.Element {    
        return (
            <Select onSelect={(choice) => this.storeSelectFilter(choice, column.accessor)} choices={['any' as string | number].concat(this.state.filterChoices[column.accessor] as (string | number)[])} />
        );
    }
    ...
    private renderMultipleChoiceSelectFilter(column: ColumnProps<T>): JSX.Element {
        ...
        return (
            <MultipleChoiceSelect
                onSelect={(choice) => this.storeMultipleChoiceSelectFilter(choice, column.accessor)}
                onDeselect={(choice) => this.unstoreMultipleChoiceSelectFilter(choice, column.accessor)}
        ...
        return this.getTableData().map((row, index) => {
            ...
            return (
                <tr className={`${!this.props.onRowClick ? 'normalCursor' : ''}`} key={'table-row-' + index} onClick={this.props.onRowClick ? onClick : undefined} >
                    {this.props.columns.map((column) => <td className={`${column.keepTextFormat ? 'lowercase' : ''} ${column.contentWidth ? 'contentWidth' : ''}`} key={`${column.accessor}-${index}`} >
    ...
    private renderHeading(column: ColumnProps<T>): JSX.Element {
        ...
        const onClick = this.props.disableActions ? undefined : () => this.sort(column.accessor);
    ...
    private renderPageFinder(): JSX.Element {
        ...
        for (let i = startNumbering; i < numRun; i++) {
            pageFinder.push((
                <button key={`pageOption-${i}`} onClick={() => this.changePage(i)} className={`pageOption ${i === this.state.currentPage ? 'selected' : ''}`} >{i + 1}</button>
            ));
        }
        ...
        pageFinder.push((
            <button key={`pageOption-lastPage`} onClick={() => this.changePage(numPages - 1)} className={`pageOption ${numPages === this.state.currentPage + 1 ? 'selected' : ''}`} >{numPages}</button>
        ));

        return (
            <div className="pageFinder" >
                <input onClick={() => this.changePage(0)} disabled={this.state.currentPage === 0} type="image" alt="previous page" src={process.env.PUBLIC_URL + '/icons/chevron-left.svg'} />
                { pageFinder }
                <input onClick={() => this.changePage(numPages - 1)} disabled={this.state.currentPage === numPages - 1} type="image" alt="next page" src={process.env.PUBLIC_URL + '/icons/chevron-right.svg'} />
            </div>
        )
    }

    private storeSelectFilter(value: string | number, accessor: T) {
    ...
    private storeMultipleChoiceSelectFilter(value: string | number, accessor: T) {
    ...
    private unstoreMultipleChoiceSelectFilter(value: string | number, accessor: T) {
        const existing = this.state.filters[accessor];
    ...
    private sortData(data: DataProps<T>[]): DataProps<T>[] {
        ...
        return data.sort((a, b) => {
            let aData = a[accessor];
            let bData = b[accessor];
