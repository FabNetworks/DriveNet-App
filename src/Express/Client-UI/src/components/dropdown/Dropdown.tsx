    ...
    public render(): JSX.Element {
        return (
            <div className={`dropdown-component ${this.state.showingMenu ? 'show' : ''} ${this.props.keepTextFormat ? 'lowercase' : ''}`} >
                <span id={this.state.id} tabIndex={0} onKeyPress={(e) => this.keyListener(e, this.showMenu.bind(this))} onClick={() => this.showMenu()} >{this.props.title}</span>
                <ul>
                    { this.props.choices.map((choice) => <li key={choice.name} tabIndex={0} onKeyPress={(e) => this.keyListener(e, choice.action)} onClick={choice.action} >{choice.name}</li>) }
                </ul>
            </div>
        );
    }
